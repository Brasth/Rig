# Rig

One parent CLI (Codex or Grok). You type a normal prompt. The parent may spawn Grok, Claude Code, Cursor, or Codex as workers. Claude and Cursor are never the parent. Missing worker binary → cheaper same-CLI. That is success.

## Install

```bash
curl -fsSL https://raw.githubusercontent.com/Brasth/Rig/main/install.sh | bash
```

No GitHub login. It clones over HTTPS, copies into `~/.rig`, puts `rig` on `~/.local/bin`, runs `rig setup`, and deletes the temp clone. Same command later updates (idempotent). It does **not** overwrite a project’s `.rig/harness.toml` or `.rig/MEMORY.md`.

From a checkout you already have: `./install.sh` (same copy + `rig setup`, no clone).

**PATH (only if `rig` is not found):**

```bash
export PATH="$HOME/.local/bin:$PATH"
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.zshrc
source ~/.zshrc
```

`which rig` must print `$HOME/.local/bin/rig`.

You need Codex CLI and/or Grok CLI as the parent. Optional worker binaries: `grok`, `claude`, `cursor-agent`, `codex`.

## Per project

```bash
cd your-repo
rig init
rig doctor
```

`rig init` is per repo. Do this in every project you want Rig to manage. Existing `.rig/harness.toml` flags are never flipped.

Fully quit Grok and Codex once after first install (not just the tab — quit the apps). MCP tools and the Grok status line load on a cold start.

Open a **new** Codex or Grok thread in that repo. An already-open session will not pick up `AGENTS.md` or skills.

Type a normal prompt in that parent CLI. Example: `fix the failing tests in tests/test_cli.py`. Do **not** use `rig run` for normal work.

## Configure

```bash
rig use grok|codex
rig parent sol|astra
rig workers grok=on|off claude=on|off codex=on|off cursor=on|off
```

```toml
parent = "codex"
# parent = "grok"

[parent]
profile = "sol"
# profile = "astra"

[workers]
codex = false
grok = true
claude = true
cursor = false
```

- **Live parent** is whichever Codex or Grok you actually opened (`rig status`). The `parent =` key is only the preferred default (`rig use grok|codex`). Opening the CLI is what makes it live.
- A worker is **effective** only when: flag true **and** binary on PATH **and** not the live parent.
- Claude Code and Cursor are never the parent.
- Grok Bot.app and Cursor.app are GUIs, **not** spawnable workers. The Cursor worker binary is `cursor-agent`.

## Watch

`rig tui` / `rig jobs` / `/rig` in Grok or Codex. Grok also gets a bottom status line after setup (restart Grok once).

Jobs and MEMORY are this repo, not the chat. A new thread still sees `.rig/jobs`. Running children keep going.

If a Claude child is `ask`, the parent answers `rig job allow <id>` or `rig job deny <id>` (TUI `y` / `n`). Never kill that job.

More: [Usage](docs/usage.md) (setup, doctor, harness, daily use, troubleshooting).
Parent spawn protocol: `.agents/skills/delegate-harness/SKILL.md` (also the `<!-- rig:start -->` block in `AGENTS.md`).
