...
Pick compares eligible model+effort profiles by minimum sufficient tier, configured preference, then stable profile ID. Default fast/standard worker order: Codex, Grok, Claude, OpenCode, OMP, Pi, agy. Strong/review: Claude, Grok, OpenCode, OMP, Pi, agy, Codex. Devin SWE-2 profiles are omitted from those defaults; list `devin-swe-2-medium` / `devin-swe-2-high` / `devin-swe-2-max` in `.rig/routing.json` preferences to opt in. Flags, binary availability, job-scoped MCP readiness, live-parent exclusion, explicit excludes, model bans, read-only explore/review, and independent-provider checks remain hard constraints. Cursor remains excluded. OMP precedes Pi by default, not by overriding configured preferences.
