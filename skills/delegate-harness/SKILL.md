...
- Wait: one blocking `rig_job_wait` with no timeout for observable wrappers (`ids` for every live wrapper); adaptive workflows use `rig_workflow_wait` (COORDINATION and ASK); native agents use host-native wait/interrupt and authenticated completion
- Child (`RIG_JOB_ID` set): **first** `rig_job_inbox` (handshake connected/time/protocol 1; no success without it; fail exact `child MCP handshake missing` preserving evidence/ownership). Permission bootstrap does not count as handshake. Legacy/unknown jobs are not retroactively failed. Then `rig_job_doing` / `rig_job_note` / `rig_job_ask` / own `rig_job_show` / `rig_memory` / `rig_job_coordination_request`. Inbox is not ASK and does not wake wait. Do not run the `rig` CLI. Do not pick, wait, spawn, queue, or allow. Children never spawn or message children. Cursor is temporarily excluded even when binary/flag are on.
...
So: Codex parent → Grok/Claude/Cursor/OpenCode/OMP/Pi/agy/Devin can be children. Grok parent → Grok child is off; the others can be children. OpenCode/OMP/Pi/agy parent → that CLI is off as a child; Grok/Claude/others can be children. Devin is never a parent. Missing binary: that worker is off for this session. Smart routing compares all eligible profiles; OMP precedes Pi by default. Devin is not in the default preference lists; opt in via `.rig/routing.json`. If no eligible wrapper exists, preserve the actual parent model/effort or mark it unknown.
...
Pick compares eligible model+effort profiles by minimum sufficient tier, configured preference, then stable profile ID. Default fast/standard worker order: Grok, Claude, OpenCode, OMP, Pi, agy, Codex. Strong/review: Claude, Grok, OpenCode, OMP, Pi, agy, Codex. Devin SWE-2 profiles are omitted from those defaults; list `devin-swe-2-medium` / `devin-swe-2-high` / `devin-swe-2-max` in `.rig/routing.json` preferences to opt in. Flags, binary availability, job-scoped MCP readiness, live-parent exclusion, explicit excludes, model bans, read-only explore/review, and independent-provider checks remain hard constraints. Cursor remains excluded. OMP precedes Pi by default, not by overriding configured preferences.
...
3. One normal blocking MCP `rig_job_wait` for observable wrapper work (no timeout), until ASK, result, cancellation, or an explicit reconciliation outcome. Adaptive workflows: `rig_workflow_wait` (COORDINATION and ASK; sibling node status). After implement+verify ok, one wait on wrapper review+seed IDs together (`ids`) only when file AND resource disjoint. Native agents use their host's wait/interrupt and authenticated completion. Do not parse a TUI. If MCP wait errors or the transport drops the tool, take one bounded `rig job wait ID --timeout 0` snapshot and inspect/reconcile. Never blindly re-wait, and never use this fallback after explicit cancellation.
