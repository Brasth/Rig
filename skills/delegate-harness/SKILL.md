...
description: >
  ...
  bash `rig pick`) and spawn a Rig worker for code, review, fix, SSH, remote
  debug, or gather in the codebase. Parent must not write the fix; parent checks
  first and writes the brief. Delegate implement, review, explore, or split work
...
- write the listed files and changes; do not hunt extra updates
